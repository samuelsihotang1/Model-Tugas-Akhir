...
_, mf_flops, _ = calculate_flops(model=MobileFormer(config_294), input_shape=input_shape, print_results=False, output_as_string=False)
_, co_flops, _ = calculate_flops(model=CoAtNet(model_image_size[1], model_image_size[2], model_image_size[0], config=f'coatnet-3', num_classes=num_classes), input_shape=input_shape, print_results=False, output_as_string=False)
_, mfra_flops, _ = calculate_flops(model=MobileFormerRA(config_294), input_shape=input_shape, print_results=False, output_as_string=False)

#@title Result
print(f'Mobile-Former FLOPs : {mf_flops:.2f}')
print(f'CoAtNet FLOPs : {co_flops:.2f}')
print(f'Mobile-Former-RA FLOPs : {mfra_flops:.2f}')